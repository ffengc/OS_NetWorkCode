    ...
    for (int i = 0; i < PROCESS_NUM; i++)
    {
        ...
        if (id == 0)
        {
            ...
            while (true)
            {
                ...
                // 现在子进程就要等待父进程的命令
                bool quit = false;
                int command = waitCommand(pipefd[0], quit); // 如果对方不发命令，我们就阻塞
                // 如果quit是true --- 直接break
                if (quit == true)
                    break;
                // 执行对应的命令
    ...
    while (true)
    {
        ...
        else if (select == 2)
        {
            ...
            while (true)
            {
                ...
                // 布置任务，把任务给指定的进程
                sendAndWakeUp(slots[choice].first, slots[choice].second, command); // 发送命令，同时把子进程唤醒，让它去干活了
                sleep(1);
            }
        }
    }
